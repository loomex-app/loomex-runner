...
Protocol input and output changes require synchronized plugin schemas, new digests in the plugin pin, compatibility review, and cross-product validation. `workflows.create` may include an optional workflow `definition` and `notes`; when supplied, the backend creates the workflow and its canonical draft atomically and returns that draft. Creation without a definition remains supported and returns `draft: null`. This authoring path records workflow definitions; node providers are selected only when a prepared workflow run executes. Backend response fields outside a local method's declared result are folded into `details`; required local fields must still be present. A local result too large for one frame becomes a checksummed response reference that can be read in 262,144-byte pages.
...
HTTP jobs bind `resultContract` to `loomex.http-result/v1` and advertise that contract in the runner manifest. The deadline begins before DNS resolution. A DNS or pre-send deadline produces `HTTP_REQUEST_TIMEOUT` with `dispatchState: "known_not_dispatched"`; the send future's first poll is the dispatch boundary, after which cancellation, deadline, or transport failures are `HTTP_REQUEST_INDETERMINATE`. The runner writes the response body to the owner-only job directory while reading it. A result whose serialized JSON is under 262,144 bytes and whose body is safe text/JSON remains inline with `bodyStorage: "inline"`. Binary or larger responses are uploaded through the regular resumable artifact transfer before terminal delivery, then use `bodyStorage: "artifact"` and a finalized `bodyRef` carrying `loomex.http-response-artifact/v1`, artifact ID, name, byte length, SHA-256, and content type.

The exact prepared provider contracts and fixture-versus-external qualification boundary are recorded in [provider qualification evidence](provider-qualification.md).
