...
`preparations.get {preparationId}` reads an existing owner-only sealed preparation without contacting the backend, preparing again, committing, or changing authorization. The runner first verifies the selected organization, authenticated child runner subject, installation, live workspace grant, unchanged provider snapshot, sealed binding identity, expiry, and lack of a prior commit authorization. A sealed preparation always remains stale. When its one authorized commit has already produced a durably recorded execution, the stale `commit_started` projection includes only that `executionId`; a client must still read that execution before displaying it. This recovery link never makes the preparation reusable or authorizes another commit.
